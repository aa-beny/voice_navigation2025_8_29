...
class TurtleBot4Navigator(BasicNavigator):
    ...
    def startThroughPoses(self, poses):
        """
        執行通過多個姿態點的導航動作並顯示回饋訊息。

        :param poses: 目標姿態點列表。
        """
        i = 0  # 計數器，用於控制回饋訊息顯示頻率
        self.goThroughPoses(poses)  # 開始執行通過多個姿態點的導航任務

        # 當任務尚未完成時持續監控
        while not self.isTaskComplete():
            feedback = self.getFeedback()  # 獲取導航回饋訊息
            i = i + 1  # 增加計數器
            # 每5次迴圈顯示一次預估到達時間
            if feedback and i % 5 == 0:
                print('Estimated time of arrival: ' + '{0:.0f}'.format(
                ...
                # 導航超時檢查（600秒後取消任務）
                if Duration.from_msg(feedback.navigation_time) > Duration(seconds=600.0):
                    self.cancelTask()  # 取消導航任務

        # 獲取最終結果並根據狀態顯示相應訊息
        result = self.getResult()
        if result == TaskResult.SUCCEEDED:
            self.info('Goal succeeded!')  # 目標達成！
            # 發布導航狀態到topic
            status_msg = String()
            status_msg.data = "navigation_succeeded"
            self.nav_status_publisher.publish(status_msg)
        elif result == TaskResult.CANCELED:
            self.info('Goal was canceled!')  # 目標被取消！
            # 發布導航狀態到topic
            status_msg = String()
            status_msg.data = "navigation_canceled"
            self.nav_status_publisher.publish(status_msg)
        elif result == TaskResult.FAILED:
            self.info('Goal failed!')  # 目標失敗！
            # 發布導航狀態到topic
            status_msg = String()
            status_msg.data = "navigation_failed"
            self.nav_status_publisher.publish(status_msg)
        else:
            self.info('Goal has an invalid return status!')  # 目標狀態無效！
            # 發布導航狀態到topic
            status_msg = String()
            status_msg.data = "navigation_invalid_status"
            self.nav_status_publisher.publish(status_msg)
